...
const TaskCard = ({ task, onClick, overlay = false }) => {
  ...
  const start = formatDueDate(task.start_date);
  ...
  return (
    ...
    >
      ...
      <div className="mt-3.5 flex items-center justify-between border-t border-line/70 pt-3">
        ...
        <div className="flex items-center gap-1.5">
          {start && (
            <span className="flex items-center gap-1 rounded-full bg-surface-2 px-2 py-0.5 text-[11px] font-medium tabular text-muted">
              {start.label}
            </span>
          )}
          {due && (
