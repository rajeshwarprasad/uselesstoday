...
import {
  ...
  ChevronLeft, ChevronRight, Sparkles, Building2, ChevronDown,
} from "lucide-react";
...
const Sidebar = ({ collapsed, onToggle, onCreateBoard, onCommand }) => {
  ...
  return (
    ...
    >
      ...
      {!collapsed && currentCompany && (
        <div className="mx-3 mb-1">
          <button
            ...
            className="flex h-10 w-full items-center gap-2.5 rounded-2xl px-2.5 text-sm font-medium text-ink transition-colors hover:bg-surface-2"
          >
            <Building2 className="h-4 w-4 shrink-0 text-muted" />
            <span className="flex-1 truncate text-left">{currentCompany.name}</span>
      ...
      {collapsed && currentCompany && (
        <div className="mx-auto mb-1 flex justify-center">
          <button
            ...
            className="grid h-9 w-9 place-items-center rounded-xl text-muted transition-colors hover:bg-surface-2 hover:text-ink"
          >
      ...
      <CompanySwitcher open={companySwitcherOpen} onClose={() => setCompanySwitcherOpen(false)} />
          ...
          boards.map((b) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "flex h-11 items-center rounded-2xl text-sm transition-colors duration-200",
                    collapsed ? "mx-auto w-11 justify-center" : "gap-3 px-3",
                    isActive ? "bg-brand-50 font-semibold text-brand-700" : "text-ink hover:bg-surface-2"
                  )
                }
              >
                <span
                  className="grid h-8 w-8 shrink-0 place-items-center rounded-xl font-display text-[13px] font-bold"
                  style={{ backgroundColor: `${color}22`, color }}
                >
                  {b.title?.[0]?.toUpperCase() || "B"}
                </span>
                {!collapsed && <span className="flex-1 truncate font-medium">{b.title}</span>}
                {!collapsed && (
                  <span className="shrink-0 rounded-full bg-surface-2 px-2 py-0.5 text-[10px] font-semibold tabular text-muted">{b.task_count}</span>
                )}
              </NavLink>
            );
          })
