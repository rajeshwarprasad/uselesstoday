...
const Calendar = () => {
  ...
  const byDay = useMemo(() => {
    ...
    tasks.forEach((t) => {
      if (t.due_date) {
        ...
        (map[key] ||= []).push({ ...t, _type: "due" });
      }
      if (t.start_date) {
        const key = format(new Date(t.start_date), "yyyy-MM-dd");
        (map[key] ||= []).push({ ...t, _type: "start" });
      }
    });
    return map;
  }, [tasks]);
  ...
  const monthCount = useMemo(
    () => tasks.filter((t) => (t.due_date || t.start_date) && isSameMonth(new Date(t.due_date || t.start_date), cursor)).length,
    [tasks, cursor]
  );

  return (
    <>
      <Topbar title="Calendar" subtitle="Tasks by start & due date" onCreateBoard={openCreateBoard} />
        ...
        <div className="mx-auto max-w-[1600px] px-6 py-8 md:px-8">
                      ...
                      <div className="space-y-1">
                        {items.slice(0, 3).map((t) => (
                          <Link
                            key={`${t.id}-${t._type}`}
                            to={`/board/${t.board_id}`}
                            title={`${t.title} · ${t._type === "start" ? "Starts" : "Due"} · ${t.board_title}`}
                            className={cn(
                              "flex items-center gap-1.5 rounded-md px-1.5 py-1 text-[11px] font-medium text-ink transition-colors hover:bg-elevated",
                              t._type === "start" ? "border-l-2 border-sky-400 bg-sky-50/60" : "bg-surface-2"
                            )}
                          >
                            <span
                            ...
                            {t._type === "start" && <span className="ml-auto shrink-0 text-[9px] text-sky-500">start</span>}
                          </Link>
                        ))}
                        {items.length > 3 && (
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4">
            ...
            <span className="flex items-center gap-1.5 text-xs text-muted">
              <span className="h-2 w-2 rounded-sm border-l-2 border-sky-400 bg-sky-50" /> Start date
            </span>
            <span className="flex items-center gap-1.5 text-xs text-muted">
              <span className="h-2 w-2 rounded-sm bg-surface-2" /> Due date
            </span>
