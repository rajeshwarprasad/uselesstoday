...
const Dashboard = () => {
  ...
  return (
    <>
      ...
      <Topbar
        title="Dashboard"
        subtitle={currentCompany?.name ? <><span className="font-semibold text-priority-urgent/80">{currentCompany.name}</span> workspace</> : "Your boards and shared projects"}
        onCreateBoard={openCreateBoard}
        actions={
          <Button size="md" variant="outline" onClick={() => setInviteOpen(true)} className="hidden sm:inline-flex">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-[1600px] px-6 py-8 md:px-8">
          ...
          <div className="mb-8 flex items-end justify-between">
            ...
            <Button size="sm" variant="outline" onClick={() => setCompanySwitcherOpen(true)}>
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {boards.map((b, i) => (
                ...
                >
                  <Link
                    to={`/board/${b.id}`}
                    className="group relative block h-full overflow-hidden rounded-3xl border-2 border-line bg-surface p-6 shadow-[var(--shadow-card)] transition-all duration-300 hover:shadow-[var(--shadow-soft)] hover:border-brand-300"
                  >
                    {/* color accent strip */}
                    <span
                    ...
                    <h4 className="font-display text-lg font-bold tracking-tight transition-colors group-hover:text-brand-600">
                      {b.title}
                    </h4>
