...
import { Sparkles, FileText, Users, Activity, Search, ChevronLeft, Printer } from "lucide-react";
...
import BoardColorModal from "../components/board/BoardColorModal";
...
const BoardPage = () => {
  ...
  const actions = (
    <div className="flex items-center gap-2">
      ...
      <Button size="sm" variant="ghost" onClick={() => window.print()} title="Print board">
  ...
  return (
    <>
        ...
        title={
          <span className="flex items-center gap-2">
            ...
            {b.board ? (
              <>
                <button
                  onClick={() => isOwner && setColorOpen(true)}
                  className={`h-2.5 w-2.5 rounded-full transition-transform ${isOwner ? "hover:scale-150 cursor-pointer" : "cursor-default"}`}
      ...
      {/* Filter bar */}
      <div className="flex flex-wrap items-center gap-2.5 px-6 py-3.5" data-print="filter">
      ...
      <div className="print-header hidden">
        ...
        {b.board?.description && <p className="mt-0.5 text-xs text-muted">{b.board.description}</p>}
        <p className="mt-1 text-[10px] text-faint">
          Printed on {new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
        </p>
      </div>

      <div className="flex-1 overflow-hidden pt-4" data-print="board">
        {b.loading ? (
      ...
      <TaskModal
        ...
        companyId={b.board?.company_id}
        onTaskMoved={() => setTaskModal({ open: false, task: null, columnId: null })}
      />
      <AIGenerateModal
        open={aiGen.open}
      ...
      <BoardColorModal
        ...
        currentColor={b.board?.color || "#6366f1"}
        onSave={(data) => b.updateBoard(data)}
      />
    </>
  );
};
